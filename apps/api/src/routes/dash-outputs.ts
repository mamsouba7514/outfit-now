...
export async function dashOutputRoutes(app: FastifyInstance) {
  ...
  app.post<{ Params: { id: string } }>('/v1/dash/outputs/:id/validate', auth, async (request, reply) => {
  ...
  app.post<{ Params: { id: string } }>('/v1/dash/outputs/:id/reject', auth, async (request, reply) => {
    const output = await prisma.dashOutput.findUnique({ where: { id: request.params.id } });
    if (!output) return reply.status(404).send({ message: 'Output not found' });
    await prisma.dashTask.update({ where: { id: output.taskId }, data: { status: 'IN_PROGRESS' } });
