...
function ScoreBar({ score }: { score: number }) {
  ...
  const color = pct >= 80 ? colors.primary[400] : pct >= 60 ? colors.neutral[400] : colors.neutral[600];
  ...
  return (
    <View style={scoreBarStyles.wrap}>
      <Animated.View style={[scoreBarStyles.fill, { width: animatedWidth, backgroundColor: color }]} />
    </View>
  );
}
...
function ActionCard({ children, onPress, tall }: { children: React.ReactNode; onPress: () => void; tall?: boolean }) {
...
export default function HomeScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const [dressingRes, outfitsRes, savedRes] = await Promise.all([
    ...
    >
      ...
      {outfits.length > 0 && (
        <>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.outfitsScroll} contentContainerStyle={{ gap: 12, paddingRight: spacing[4] }}>
            {outfits.map((outfit) => (
      ...
      {!loading && dressingCount === 0 && (
        <View style={styles.emptyBanner}>
          <Ionicons name="shirt-outline" size={36} color={colors.neutral[700]} style={{ alignSelf: 'center', marginBottom: spacing[2] }} />
          <Text style={styles.emptyBannerTitle}>TON DRESSING EST VIDE</Text>
          <Text style={styles.emptyBannerSub}>Commence par scanner ta première pièce pour que l'IA puisse composer tes tenues.</Text>
          <TouchableOpacity style={styles.emptyBannerBtn} onPress={() => router.push('/(app)/scan')} activeOpacity={0.85}>
  ...
  return (
    <Animated.View style={[outfitCardStyles.card, { width: cardW, transform: [{ scale }] }]}>
      <TouchableOpacity onPressIn={onIn} onPressOut={onOut} activeOpacity={1}>
        {firstItem?.imageUrl ? (
          <Image source={{ uri: firstItem.imageUrl }} style={outfitCardStyles.img} contentFit="cover" />
        ) : (
...
const styles = StyleSheet.create({
  ...
  divider: { height: 2, backgroundColor: colors.rose?.[500] ?? '#E8194A', width: 40, marginBottom: spacing[5] },
  statsRow: {
    flexDirection: 'row',
    backgroundColor: colors.neutral[900],
  ...
  sectionLabel: { fontSize: 10, color: colors.neutral[500], letterSpacing: 3, marginBottom: spacing[3] },
  sectionHeader: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
